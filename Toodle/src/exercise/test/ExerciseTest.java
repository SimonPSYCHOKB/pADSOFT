...
public class ExerciseTest {
	...
	public void testGetDateOfEnd() {
		assertTrue(! (new Date(LocalDateTime.now().plusDays(10).atZone(ZoneId.systemDefault()).toInstant().toEpochMilli())).after(e.getDateOfEnd()));
		assertTrue(! (new Date(LocalDateTime.now().plusDays(10).atZone(ZoneId.systemDefault()).toInstant().toEpochMilli())).before(e.getDateOfEnd()));
	}
	...
	public void testBeginExercise() {
		e.beginExercise(s);
		System.out.println(s.getCurrentExercises());
